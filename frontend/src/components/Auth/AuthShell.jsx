...
const AuthShell = ({
  ...
  return (
    <div className="relative min-h-[100dvh] overflow-hidden bg-[#081320] text-white">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100dvh] w-full max-w-7xl flex-col px-4 py-6 sm:px-6 lg:flex-row lg:items-center lg:gap-12 lg:px-8 lg:py-10">
        <motion.div
          initial={{ opacity: 0, y: 18 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.55 }}
          className="flex-1 px-2 py-8 lg:px-4"
        >
          <Link
            to="/"
          ...
          <div className="mt-8 max-w-xl">
            ...
            <h1 className="mt-4 font-['Georgia'] text-4xl font-semibold leading-[1.02] text-white sm:text-5xl lg:text-6xl">
              A learning space that feels focused, warm, and human.
            </h1>
            <p className="mt-5 max-w-lg text-base leading-7 text-[rgba(233,239,247,0.76)] sm:text-lg">
              The mobile app already has that mood. This flow brings the same identity to web so
              signing in feels like entering LangVoyage, not a generic dashboard.
            </p>
          </div>
        ...
        >
          <div className="rounded-[2rem] border border-white/12 bg-[linear-gradient(180deg,rgba(248,251,255,0.96),rgba(239,246,252,0.94))] p-5 shadow-[0_24px_80px_rgba(3,11,20,0.34)] sm:p-7 lg:p-9">
            <div className="mb-6">
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-[#1b7f79]">
                {eyebrow}
              </p>
              <h2 className="mt-3 font-['Georgia'] text-3xl font-semibold leading-tight text-[#14314a] sm:text-4xl">
                {title}
              </h2>
              <p className="mt-3 max-w-xl text-sm leading-6 text-[#5f7085] sm:text-base">
                {description}
              </p>
            </div>
            {children}
